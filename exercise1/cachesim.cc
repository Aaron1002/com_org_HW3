...
{
  size_t idx = (addr >> idx_shift) & (sets-1);  // # of sets
...
{
  store ? write_accesses++ : read_accesses++; // if invoke the write operation, write_access++
  (store ? bytes_written : bytes_read) += bytes;  // set the read/write size

  uint64_t* hit_way = check_tag(addr);  // check tag
  if (likely(hit_way != NULL))  // cache hit
  {
    if (store)  // if write operation == 1
      *hit_way |= DIRTY;
    return;
  }

  store ? write_misses++ : read_misses++; // cache miss
  if (log)
  {
    std::cerr << name << " "
  ...
  if ((victim & (VALID | DIRTY)) == (VALID | DIRTY)) // dirty bit == 1 (writeback)
  {
